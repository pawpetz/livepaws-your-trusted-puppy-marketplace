...
function BreederDashboardPage() {
  ...
  const handleMediaSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNewPet((prev) => ({ ...prev, media: [...prev.media, ...items] }));
    e.target.value = '';
  };

  const removeNewPetMedia = (index: number) => {
    setNewPet((prev) => ({ ...prev, media: prev.media.filter((_, i) => i !== index) }));
  };
  ...
  const handleAddPet = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const coverImage = newPet.media.find((m) => m.type === 'image')?.url;
    await addPet({
      data: {
  ...
  const handleSaveEdit = async () => {
    ...
    await updatePetMedia({ data: { id: editingPet.id, media: editingPet.media } });
    await refresh();
    setSavingEdit(false);
    setEditingPet(null);
  };
              ...
              <Card className="p-6">
                ...
                <form onSubmit={handleAddPet} className="space-y-3 pt-4 text-xs">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <label className="flex cursor-pointer items-center gap-2 rounded-xl border border-dashed border-border bg-secondary/30 p-3 transition-colors hover:border-primary">
                      <ImagePlus size={18} className="text-muted-foreground" />
                      <span className="text-muted-foreground">Add photos or videos (multiple allowed)</span>
                      <input
                        type="file"
                        accept="image/*,video/*"
                        multiple
                    ...
                    {newPet.media.length > 0 && (
                      <div className="grid grid-cols-4 gap-2 pt-1">
                        {newPet.media.map((m, i) => (
              ...
              <DialogContent>
                ...
                {editingPet && (
                  <div className="space-y-3 text-xs">
                    ...
                    <div className="space-y-1.5">
                      <Label>Photos &amp; videos</Label>
                      <label className="flex cursor-pointer items-center gap-2 rounded-xl border border-dashed border-border bg-secondary/30 p-2.5 transition-colors hover:border-primary">
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            setEditingPet({ ...editingPet, media: [...editingPet.media, ...items] });
                            e.target.value = '';
                          }}
                        />
                      </label>
                      {editingPet.media.length > 0 && (
                        <div className="grid grid-cols-4 gap-2 pt-1">
                          {editingPet.media.map((m, i) => (
                            <div key={i} className="group relative aspect-square overflow-hidden rounded-lg border border-border">
                              {m.type === 'video' ? (
                                <video src={m.url} className="h-full w-full object-cover" muted />
                              ) : (
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  setEditingPet({ ...editingPet, media: editingPet.media.filter((_, idx) => idx !== i) })
                                }
                                className="absolute right-0.5 top-0.5 rounded-full bg-black/60 p-0.5 text-white opacity-0 transition-opacity group-hover:opacity-100"
                              >
                                <X size={12} />
